...
class ProfileController extends Controller
{
    ...
    public function profileVacancy(Vacancy $vacancy){
        ...
        foreach(CandidateFieldGroup::where('public',1)->orderBy('sort_order')->get() as $group){
            foreach($group->candidateFields as $field){
                if($field->required==1){
                    if ($user->candidateFields()->where('id', $field->id)->first()) {
                        $value = $user->candidateFields()->where('id', $field->id)->first()->pivot->value;
                        if(!isset($value))
                        $rules['field_'.$field->id] = 'required';
                    }
                }
        ...
        if(count($rules) == 0 && isset($user->name) && isset($user->candidate->display_name) && isset($user->candidate->gender) && isset($user->candidate->date_of_birth) && isset($user->email)){
            Application::create([
            ...
            $admin_message = __('site.apply_vacancy_to_admin',[
                ...
                'location' => $vacancy->location,
                'closes_at' => $vacancy->closes_at,
                'application-records' => $admin_link,
            ]);
            if($vacancy->user){
            ...
            return redirect()->route('candidate.vacancy.complete')->with('flash_message', __('site.changes-saved-vacancy'));
        }
        ...
        return view('candidate.profile.apply', compact('candidate', 'kvk_flag', 'paychecked_flag', 'niwo_flag', 'vacancy'));
    }

    public function updateProfileVacancy(Request $request, Vacancy $vacancy){
        $user = Auth::user();
        if($user->candidate->locked==1){
            return back()->with('flash_message',__('site.profile-locked'));
        }
        ...
        // if(count($rules) == 9 && isset($user->candidate->name) && isset($user->candidate->display_name) && isset($user->candidate->gender) && isset($user->candidate->date_of_birth) && isset($user->candidate->email)){
        //     Application::create([
        //         'vacancy_id'=>$vacancy->id,
        //         'user_id'=>$user->id,
        //     ]);
        ...
        return redirect()->route('candidate.vacancy.complete')->with('flash_message', __('site.changes-saved-vacancy'));
        // return back()->with('flash_message', __('site.changes-saved-vacancy'));
    }
}
