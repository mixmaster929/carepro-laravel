...
?>
@section('page-title',__('site.order').' #'.$order->id.$offer1)
@section('breadcrumb')
    <li  class="breadcrumb-item"><a href="{{ route('candidate.orders') }}">@lang('site.orders')</a></li>
    ...
    <div class="card bd-0">
        <div class="card-header bg-gray-400 bd-b-0-f pd-b-0">
            <nav class="nav nav-tabs">
                ...
                <a id="commentTab" class="nav-link" data-toggle="tab" href="#tabCont4">@lang('site.comments') ({{ $comments->count() }})</a>
        ...
        <div class="card-body bd bd-t-0 tab-content">
            <div id="tabCont1" class="tab-pane active show">
                ...
                @if(!($pivotData && ($pivotData->pivot->status == 'allow')))
                <button type="button" class="btn btn-primary btn-block rounded int_mt30px" data-toggle="modal" data-target="#exampleModal">
                    {{ $offer? trans('site.edit-offer') : trans('site.make-offer') }}
                </button>
                @endif
            ...
            <div id="tabCont4" class="tab-pane">
                <form action="{{ route('candidate.order.add-comment',['order' => $order->id, 'user' => Auth::user()->id]) }}" method="post">
                    @csrf
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-body">
                            <form id="make-offer" method="POST" action="{{ route('candidate.offer-apply') }}">
                                @csrf
                                <span>My Offer</span>
                                <span>€</span>
                                <input type="input" value="{{ $pivotData? $pivotData->pivot->offer : '' }}" placeholder="amount" name="amount" required></input>
                                <input type="input" value="{{ $order->id }}" name="order_id" hidden></input>
                            </div>
                            <div class="modal-footer">
...
@section('footer')
    <script>
    "use strict";
        $('#comment-box').load('{{ route('candidate.order.comments',['order'=>$order->id])  }}');
