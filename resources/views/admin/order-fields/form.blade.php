<div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}">
    ...
    <input required class="form-control" name="name" type="text" id="name" value="{{ old('name',isset($orderfield->name) ? $orderfield->name : '') }}" >
...
<div class="form-group {{ $errors->has('type') ? 'has-error' : ''}}">
    ...
    <select required name="type" class="form-control" id="type" >
        <option value=""></option>
        @foreach (json_decode('{"text":"Text","textarea":"Textarea","select":"Select","radio":"Radio","checkbox":"Checkbox","file":"File","label":"Label"}', true) as $optionKey => $optionValue)
            <option value="{{ $optionKey }}" {{ (isset($orderfield->type) && $orderfield->type == $optionKey) ? 'selected' : ''}}>{{ $optionValue }}</option>
        @endforeach
    </select>
...
<div class="form-group {{ $errors->has('sort_order') ? 'has-error' : ''}}">
    ...
    <input required class="form-control number" name="sort_order" type="text" id="sort_order" value="{{ old('sort_order',isset($orderfield->sort_order) ? $orderfield->sort_order : $sortOrder) }}" >
...
<div id="option-container" class="form-group {{ $errors->has('options') ? 'has-error' : ''}}">
    ...
    <textarea placeholder="@lang('site.option-placeholder')" class="form-control" rows="5" name="options" type="textarea" id="options" >{{ isset($orderfield->options) ? $orderfield->options : ''}}</textarea>
...
<div class="form-group {{ $errors->has('required') ? 'has-error' : ''}}">
    ...
    <select name="required" class="form-control" id="required" >
        @foreach (json_decode('{"1":"'.__('site.yes').'","0":"'.__('site.no').'"}', true) as $optionKey => $optionValue)
            <option value="{{ $optionKey }}" {{ (isset($orderfield->required) && $orderfield->required == $optionKey) ? 'selected' : ''}}>{{ $optionValue }}</option>
        @endforeach
    </select>
...
<div class="form-group {{ $errors->has('placeholder') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="placeholder" type="text" id="placeholder"  value="{{ isset($orderfield->placeholder) ? $orderfield->placeholder : ''}}" >
...
<div class="form-group {{ $errors->has('enabled') ? 'has-error' : ''}}">
    ...
    <select name="enabled" class="form-control" id="enabled" >
        @foreach (json_decode('{"1":"'.__('site.yes').'","0":"'.__('site.no').'"}', true) as $optionKey => $optionValue)
            <option value="{{ $optionKey }}" {{ (isset($orderfield->enabled) && $orderfield->enabled == $optionKey) ? 'selected' : ''}}>{{ $optionValue }}</option>
        @endforeach
    </select>
