...
@section('search-form',url('/admin/sms'))

@section('pageTitle',__('site.text-messages'))

@section('page-title')
    {{ __('site.text-messages') }}
            ...
            <div class="col-md-12">
                <div >
                    <div>
                        ...
                        <a href="{{ url('/admin/sms/create') }}" class="btn btn-success btn-sm" title="@lang('site.add-new')">
                        ...
                        <a  href="{{ route('admin.sms.index') }}?sent={{ request()->sent }}" class="btn btn-info btn-sm" title="@lang('site.reset')">
                        ...
                        <form id="msg-list" action="{{ route('admin.sms.delete-multiple') }}" method="post">
                            @csrf
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <thead>
                                    <tr>
                                        <th>#</th><th>@lang('site.message')</th>
                                        <th>@lang('site.gateway')</th>
                                        <th>@lang('site.recipients')</th>
                                        <th>@lang('site.sent')</th>
                                        <th>@lang('site.send-date')</th><th>@lang('site.actions')</th>
                                    </tr>
                                </thead>
                                <tbody>
                                @foreach($sms as $item)
                                    <tr>
                                        ...
                                        <td>{{ limitLength($item->message,80) }}</td>
                                        <td>{{ $item->smsGateway->gateway_name }}</td>
                                        <td>{{ $item->smsRecipients()->count() }}</td>
                                        <td>{{ boolToString($item->sent) }}</td>
                                        ...
                                        <td>
                                            ...
                                            <div class="btn-group dropup">
                                                ...
                                                <div class="dropdown-menu">
                                                    ...
                                                    <a class="dropdown-item" href="{{ url('/admin/sms/' . $item->id) }}">@lang('site.view')</a>
                                                     @endcan

                                                    @can('access','edit_text_message')
                                                    @if($item->sent==0)
                                                    <a class="dropdown-item" href="{{ url('/admin/sms/' . $item->id . '/edit') }}">@lang('site.edit')</a>
                                                    @endif
                                                    @endcan

                                                    @can('access','delete_text_message')
                                                    <a class="dropdown-item" href="{{ route('admin.sms.delete',['id'=>$item->id]) }}" onclick="return confirm(&quot;@lang('site.confirm-delete')&quot;)"   >@lang('site.delete')</a>
                                                    @endcan

                                                    @can('access','create_text_message')
                                                    @if($item->sent==0)
                                                        <a onclick="return confirm('@lang('site.send-confirm')')" class="dropdown-item" href="{{ route('admin.sms.send-now',['sms'=>$item->id]) }}">@lang('site.send-now')</a>
                                                    @endif
                                                    @endcan
                            ...
                            <div class="pagination-wrapper"> {!! clean( $sms->appends(request()->input())->render() ) !!} </div>
                        </div>
                    </form>
                    </div>
                </div>
