...
@section('pageTitle',__('site.job-category').': '.$jobcategory->name)
@section('page-title',__('site.job-category').': '.$jobcategory->name)
                ...
                <div  >
                    <div  >
                       ...
                       @endcan
                        ...
                        <a href="{{ url('/admin/job-categories/' . $jobcategory->id . '/edit') }}" ><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> @lang('site.edit')</button></a>
                        @endcan

                        @can('access','delete_vacancy_category')
                        <form method="POST" action="{{ url('admin/job-categories' . '/' . $jobcategory->id) }}" accept-charset="UTF-8" class="int_inlinedisp">
                            {{ method_field('DELETE') }}
                            {{ csrf_field() }}
                        ...
                        <ul class="list-group">
                            ...
                            <li class="list-group-item">{{ $jobcategory->id }}</li>
                            <li class="list-group-item active">@lang('site.name')</li>
                            <li class="list-group-item">{{ $jobcategory->name }}</li>
                            <li class="list-group-item active">@lang('site.sort-order')</li>
                            <li class="list-group-item">{{ $jobcategory->sort_order }}</li>
