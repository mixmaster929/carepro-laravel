...
@section('breadcrumb')
    @include('partials.breadcrumb',['crumbs'=>[
            ...
            [
                'link'=>route('employer.test-questions.index',['test'=>$testquestion->test_id]),
...
@section('content')
    <div>
        <a href="{{ route('employer.test-questions.index',['test'=>$testquestion->test_id]) }}" title="@lang('site.back')"><button class="btn btn-warning btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> @lang('site.back')</button></a>
        <br />
        <br />
        <form method="POST" action="{{ url('/employer/test-questions/' . $testquestion->id) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
            {{ method_field('PATCH') }}
            {{ csrf_field() }}
            @include ('employer.test-questions.form', ['formMode' => 'edit'])
            <br/>
            ...
            <table class="table">
                ...
                @foreach($testquestion->testOptions as $option)
                    <tr>
                        ...
                        <td>
                            <a onclick="return confirm('@lang('site.confirm-delete')')" class="btn btn-sm btn-primary" href="{{ route('employer.test-options.delete',['testOption'=>$option->id]) }}"><i class="fa fa-trash"></i>
                                @lang('site.delete')
                            </a>
                            <a data-href="{{ route('employer.test-options.edit',['testOption'=>$option->id]) }}" class="btn btn-success btn-sm editbtn" href="#"  data-toggle="modal" data-target="#editModal" >
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <form action="{{ route('employer.test-options.store',['testQuestion'=>$testquestion->id]) }}" method="post">
                    @csrf
                <div class="modal-content">
                    <div class="modal-header">
