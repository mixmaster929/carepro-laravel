...
@section('content')
    <div >
        ...
        <br />
        <br />
        <form method="POST" action="{{ route('employer.order.store-employment') }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
            {{ csrf_field() }}
            ...
            <div class="form-group {{ $errors->has('user_id') ? 'has-error' : ''}}">
                ...
                <input type="text" readonly  name="user_name" id="user_name" class="form-control" value="{{ \App\User::find($user->id)->name }}" />
                <input type="hidden" name="user_id" id="user_id" class="form-control" value="{{ \App\User::find($user->id)->id }}" />
            </div>
