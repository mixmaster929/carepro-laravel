...
@section('content')
    <div >
        ...
        <br />
        <br />
        <form method="POST" action="{{ route('employer.store-employment') }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
            {{ csrf_field() }}
            ...
            <div class="form-group {{ $errors->has('employer_user_id') ? 'has-error' : ''}}">
                ...
                <input required type="text" readonly  name="employer_user_name" id="employer_user_name" class="form-control" value="{{ $employer->name }}" />
                <input required type="hidden" name="employer_user_id" id="employer_user_id" class="form-control" value="{{ $employer->id }}" />
            </div>
            <div class="form-group {{ $errors->has('user_id') ? 'has-error' : ''}}">
                <label for="user_id" class="control-label">@lang('site.candidate')</label>
                <input type="text" readonly  name="user_name" id="user_name" class="form-control" value="{{ \App\User::find($application->user_id)->name }}" />
                <input type="hidden" name="user_id" id="user_id" class="form-control" value="{{ \App\User::find($application->user_id)->id }}" />
            </div>
